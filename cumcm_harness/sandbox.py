...
class Executor:
    def __init__(self,kind='docker',image='cumcm-egoharness:0.1.0',trusted_hashes=None):
        if kind not in ('docker','trusted-local'):raise ValueError(kind)
        self.kind=kind;self.image=image;self.trusted_hashes=set(trusted_hashes or [])
    def probe(self):
        if self.kind=='trusted-local':return {'backend':'TRUSTED_LOCAL_NOT_SANDBOX'}
        if not shutil.which('docker'):raise Blocked('Docker is required for untrusted generated code; no local fallback')
        r=subprocess.run(['docker','image','inspect',self.image,'--format','{{.Id}}'],capture_output=True,text=True,timeout=20)
        if r.returncode:raise Blocked(f'Docker image missing or daemon unavailable: {self.image}')
        return {'backend':'DOCKER','image_id':r.stdout.strip()}
    def execute(self,code:Path,entry:str,data:Path,out:Path,args:list[str],limits:Limits,*,answer:Path|None=None,logdir:Path|None=None):
        ...
        if any(out.iterdir()):raise Blocked('Output directory is not empty; no overwrite/reuse without verified receipt')
        ...
        if self.kind=='trusted-local':
            ...
            cmd += [self.image,'python','-B','-m','cumcm_harness.worker',f'/code/{entry}','--input','/data','--out','/out',*args]
            if answer:cmd+=['--answer','/answer']
        try:r=run_process(cmd,cwd=code,out=logdir,env=env,timeout=limits.seconds)
        finally:
        ...
        m=tree_manifest(out)
        if len(m)>1000 or sum((out/k).stat().st_size for k in m)>limits.output_bytes:raise Blocked('Output limit exceeded')
        if r['status']!='EXITED' or r['returncode']!=0:raise Blocked(f'Execution failed: {r["status"]}, rc={r["returncode"]}; {logdir}')
        return {**r,**backend,'code_digest':tree_id,'input_digest':digest(dm),'output_manifest':m}
...
def resource_gate(plan:dict,config:dict):
    ...
    if fail:raise Blocked('RESOURCE_GATE: '+'; '.join(fail))
