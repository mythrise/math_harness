...
from .common import Blocked, IntegrityError, ExecutionFailure, ScientificRejection, digest, read_json, tree_manifest, write_json
from .contracts import validate
from .sandbox import Limits


def validate_tests(report):
    if not isinstance(report,dict):raise ScientificRejection('Evaluator primitive tests report must be an object')
    ...
    if (not isinstance(cases,list) or len(cases)<3
        ...
        raise ScientificRejection('Evaluator primitive tests failed or are incomplete')
    return report
...
def run_preflight(root,store,executor,config,bundle):
    ...
    try:
        if not (code/'test_evaluator.py').is_file():
            raise ScientificRejection('Verifier must supply standalone test_evaluator.py for pre-freeze primitive tests')
        if not run('primitives','test_evaluator.py'):raise ScientificRejection('Primitive test process failed')
        ...
        for name,content in [('missing',None),('malformed','{ definitely not JSON'),
            ...
            if not run(name,'evaluate.py',answer):raise ScientificRejection('Malformed-answer process failed: '+name)
            evaluation=validate('evaluation',read_json(base/name/'out/evaluation.json'))
            reports[-1]['evaluation']=evaluation
            if evaluation['valid'] is not False:raise ScientificRejection('Evaluator accepted invalid answer: '+name)
            if not any(c['passed'] is False for c in evaluation['checks']):
                raise ScientificRejection('Rejected answer lacks a failing check: '+name)
        passed=True
    except (ScientificRejection,IntegrityError,FileNotFoundError,ValueError,TypeError,KeyError) as exc:
