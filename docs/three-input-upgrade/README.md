...
本ZIP是**带完整替换源码的升级包**，应用到你当前的完整仓库；不含全库克隆，不复制或替换原MOSAIC、OurWork、Exa凭证、旧工作区和已冻结实验。`build_full_source.py`可在你的真实完整checkout上导出升级后的完整源码ZIP。不能把本包的本地测试镜像当成完整仓库。
...
解压包放在仓库目录之外。使用已有Python 3.11+虚拟环境：
...
先校验、后写入。HEAD、旧Git blob、新SHA-256、工作区状态或路径不匹配会停止。写入前备份；测试失败保留修改和日志。没有自动stash、commit、push或联网。
...
- **DOCX**：修改普通正文，保留包内其他文件及原有公式、表格、图片、字段。混合数学段落、批注/跟踪变更等不安全区域不改，明确报告。输出`revised.docx`。
...
本包本地已测与未测内容见`TEST_REPORT_CN.md`。没有读取或使用用户密钥，没有远程提交。真实双模型＋Exa R2＋Docker完整联合验收留给实际认证环境，不能用本包的组件测试数字替代。
...
/path/to/math_harness/.venv/bin/python build_full_source.py \
  /path/to/math_harness --out /outside/repository/math_harness_v0.5.0-rc1_full.zip
```

它基于真实git跟踪文件加本次overlay，保留许可，排除凭证目录、工作区、字体文件等并出具清单。构建器不联网，不自动推送，也不把打包成功当成生产验证成功。
