...
from .fast import PolicyCodec,FastHarness

def key(codec,x): return tuple(map(int,codec.decode(x)))

def run_fast_h0(problem,codec,seed,pop_size,max_evals,config,mode='no_memory',ties_per_objective=1,harness_type=FastHarness):
    assert mode=='no_memory' and ties_per_objective==1, 'Fast path certified only for R1 single representative'
    codec=codec if isinstance(codec,PolicyCodec) else PolicyCodec(codec)
    key_cache={}
    def key(codec,x):
        raw=np.asarray(x,dtype=float).tobytes()
        if raw not in key_cache:key_cache[raw]=codec.key(x)
        return key_cache[raw]
    rng,X,F=core.initialize(problem,pop_size,seed)
    harness=harness_type(codec,seed,max_evals,config);harness.initialize(X,F)
    ...
    while evals<max_evals:
        ...
        child,chosen=harness.intervene(child,chosen,parent,donor,seen,arr,evals)
        if getattr(harness,'selected_parent_is_donor',False):
            parent=donor;pf=F[donor_id]
        cf=problem.evaluate(child)[0];evals+=1
        harness.observe(child,cf,evals)
    ...
    return core.RunResult('graph_'+mode,problem.name,seed,AX,AF,evals,[],
         {'counts':counts.tolist(),'reward_sum':rewards.tolist(),'route_log':log,
          ...
          'behavior_memory':False,'novelty_filter':True,'key_cache_size':len(key_cache),'harness':harness.diagnostics()})
