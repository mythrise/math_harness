...
CONFIGS={
 'dual24':SearchConfig(model='linear',uncertainty=0.,proposal_mode='dual',local_cap=24),
 'dual48':SearchConfig(model='linear',uncertainty=0.,proposal_mode='dual',local_cap=48),
 'dual24_refit32':SearchConfig(model='linear',uncertainty=0.,proposal_mode='dual',local_cap=24,refit=32),
 'local24_refit128':SearchConfig(model='linear',uncertainty=0.,proposal_mode='local',local_cap=24,refit=128),
...
class SearchHarness(FastHarness):
    ...
    def make_candidates(self,child,expert,parent,donor,seen):
        c=self.cfg;candidates=[child.copy()];experts=[expert];local={self.key(child)};origins=[0]
        self.selected_parent_is_donor=False
        if c.proposal_mode=='dual':
            available=[];seen_pool=set(local)
            # Combine two actual evaluated parent neighborhoods. Deduplicate
            # with deterministic first-source provenance, then sample a bounded
            # pool. No target-specific simulator parameters are accessed.
            for origin,base in enumerate([parent,donor]):
                nb=self.neighbors(base);self.neighbor_filter_checks+=len(nb)
                for k,e in nb:
                    if k not in seen and k not in seen_pool:
                        available.append((k,e,origin));seen_pool.add(k)
            take=min(c.local_cap,len(available))
            ids=self.rng.choice(len(available),take,replace=False) if take else []
            for i in ids:
                k,e,origin=available[int(i)];candidates.append(self.codec.encode(k));experts.append(e);origins.append(origin)
        elif c.proposal_mode.startswith('local'):
        ...
        self.pool_counts.append(len(candidates));self.pool_origins=origins if c.proposal_mode=='dual' else [0]*len(candidates);return np.asarray(candidates),experts
    def intervene(self,child,expert,parent,donor,seen,AF,fe):
        self.pending=None;c=self.cfg;self.selected_parent_is_donor=False
        ...
        self.selected_parent_is_donor=bool(self.pool_origins[pick])
        if self.pending:self.pending['parent_is_donor']=self.selected_parent_is_donor
