...
from .efficient import EfficientHarness

def run_nsga_residual(problem,codec,seed,pop_size,max_evals,config):
    rng,X,F=core.initialize(problem,pop_size,seed)
    h=EfficientHarness(codec,seed,max_evals,config);h.initialize(X,F)
    to_key=lambda x:tuple(map(int,codec.decode(x)));seen=set(to_key(x)for x in X)
