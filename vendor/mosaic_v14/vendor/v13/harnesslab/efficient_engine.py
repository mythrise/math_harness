...
from .efficient import EfficientHarness as ResidualHarness

def key(codec,x): return tuple(map(int,codec.decode(x)))

def run_efficient(problem,codec,seed,pop_size,max_evals,config,mode='no_memory',ties_per_objective=1):
    ...
    harness=ResidualHarness(codec,seed,max_evals,config);harness.initialize(X,F)
