...
def hashes():
 return {str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((ROOT/'mosaic14').glob('*.py'))}|{'protocol/instances.json':hashlib.sha256((ROOT/'protocol/instances.json').read_bytes()).hexdigest()}
