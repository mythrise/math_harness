...
from image_slots_v13 import apply_image_slots, list_image_slots
from config_validator_v13 import lint_config
from output_guard_v13 import validate_output
...
def render(template_id:str, config:Dict[str,Any]|None=None, mode='auto', exact_reference=False, strict=False, guard=True):
    ...
    if mode in ('reference_editable','reference_semantic','reference_semantic_image'):
        ...
        if mode in ('reference_semantic','reference_semantic_image'):
            out=apply_semantic_geometry(template_id,out,config)
            out=apply_image_slots(template_id,out,config)
        if mode == 'reference_editable':
            out=apply_image_slots(template_id,out,config)
        if guard:
