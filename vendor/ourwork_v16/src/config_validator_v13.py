...
from image_slots_v13 import list_image_slots, _resolve_path
...
def lint_config(template_id:str, config:Dict[str,Any]) -> Dict[str,Any]:
    ...
    image_slots={x['name']:x for x in list_image_slots(template_id).get('image_slots',[])}
    for name,spec in (config.get('images') or {}).items():
        if name not in image_slots:
            errors.append(f'Unknown image slot: {name}')
            continue
        if not isinstance(spec,dict):
            errors.append(f'image slot {name} must be an object with at least a path')
            continue
        path=spec.get('path')
        if not path:
            errors.append(f'image slot {name} is missing path')
        elif not _resolve_path(path).exists():
            errors.append(f'image slot {name} path does not exist: {path}')
        fit=(spec.get('fit') or image_slots[name].get('fit') or 'cover').lower()
        if fit not in {'cover','contain','stretch','auto'}:
            errors.append(f'image slot {name} has unsupported fit: {fit}')
    ...
    for name,value in (config.get('slots') or {}).items():
        ...
        target=slots[name].get('target') or {}
        ids=[target['raw_id']] if 'raw_id' in target else target.get('raw_ids',[])
        boxes=[raw[i] for i in ids if i in raw]
        if not boxes: continue
        w=max(b['x1'] for b in boxes)-min(b['x0'] for b in boxes); h=max(b['y1'] for b in boxes)-min(b['y0'] for b in boxes); fs=max(float(b.get('font_size',12)) for b in boxes)
        text=_text(value); est=len(text.replace('\n',' '))*fs*.50
        if est>w*2.8 and '\n' not in text:
            warnings.append(f'slot {name!r} is much longer than its reference box; auto-fit will shrink/wrap')
    return {'template_id':template_id,'ok':not errors,'errors':errors,'warnings':warnings}
