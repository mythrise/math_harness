...
from image_slots_v15 import apply_image_slots,list_image_slots,image_placement_report,expand_auto_images
from config_validator_v15 import lint_config
from output_guard_v15 import validate_output

def render(template_id:str,config:Dict[str,Any]|None=None,mode='auto',exact_reference=False,strict=False,guard=True):
 config=config or {};lint=lint_config(template_id,config)
 if mode in ('reference_editable','reference_semantic','reference_semantic_image'):
  ...
  out=apply_image_slots(template_id,out,config)
  if guard:
...
if __name__=='__main__':
 ...
 if a.placement_report_out:Path(a.placement_report_out).write_text(json.dumps({'assignments':expand_auto_images(a.template,cfg).get('_auto_image_assignments',[]),'placements':image_placement_report(a.template,cfg)},ensure_ascii=False,indent=2),encoding='utf-8')
