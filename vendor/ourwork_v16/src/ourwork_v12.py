...
from config_validator_v12 import lint_config
from output_guard_v12 import validate_output


def render(template_id:str, config:Dict[str,Any]|None=None, mode='auto', exact_reference=False, strict=False, guard=True):
    config=config or {}
    if mode in ('reference_editable','reference_semantic'):
        ...
        if mode=='reference_semantic':out=apply_semantic_geometry(template_id,out,config)
        if guard:
...
def save(template_id,out,config=None,mode='auto',exact_reference=False,strict=False,guard=True):
...
if __name__=='__main__':
 import argparse
 ap=argparse.ArgumentParser();ap.add_argument('--template',required=True);ap.add_argument('--config');ap.add_argument('--mode',default='reference_semantic',choices=['auto','reference','reference_editable','reference_semantic']);ap.add_argument('--out');ap.add_argument('--png');ap.add_argument('--pdf');ap.add_argument('--inventory-out');ap.add_argument('--slots-out');ap.add_argument('--lint-out');ap.add_argument('--validate-out');ap.add_argument('--strict',action='store_true');ap.add_argument('--no-guard',action='store_true');ap.add_argument('--exact-reference',action='store_true')
