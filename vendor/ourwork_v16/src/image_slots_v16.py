...
def _registry() -> Dict[str, Any]:
    p = ROOT / 'registry' / 'image_slots_v16.json'
...
def _smart_focus(im: Image.Image) -> Tuple[float,float,Dict[str,Any]]:
    """Estimate a crop focus using edge/contrast saliency.

    This is deliberately lightweight and deterministic; it is not an object detector.
    It works well for the common case where the subject is visually detailed and off-center.
    """
    gray=im.convert('L'); gray.thumbnail((160,160),Image.Resampling.LANCZOS)
    arr=np.asarray(gray,dtype=np.float32)
    if arr.size<16:return .5,.5,{'smart_focus_used':False,'reason':'too_small'}
    gx=np.zeros_like(arr);gy=np.zeros_like(arr);gx[:,1:-1]=np.abs(arr[:,2:]-arr[:,:-2]);gy[1:-1,:]=np.abs(arr[2:,:]-arr[:-2,:])
    sal=gx+gy
    # Favor stronger details and mildly penalize extreme borders.
    h,w=sal.shape; yy,xx=np.mgrid[0:h,0:w]; nx=(xx+.5)/w;ny=(yy+.5)/h
    prior=(0.55+0.45*(1-np.minimum(1,np.sqrt((nx-.5)**2+(ny-.5)**2)/.72)))
    sal=sal*prior
    q=float(np.percentile(sal,72));weights=np.where(sal>=q,sal,0)
    total=float(weights.sum())
    if total<1e-5:return .5,.5,{'smart_focus_used':False,'reason':'low_saliency'}
    fx=float((weights*nx).sum()/total);fy=float((weights*ny).sum()/total)
    fx=max(.12,min(.88,fx));fy=max(.12,min(.88,fy))
    return fx,fy,{'smart_focus_used':True,'smart_focus':[round(fx,4),round(fy,4)]}
...
def _fragment(template_id:str,slot:Dict[str,Any],spec:Dict[str,Any]) -> Tuple[str,str,Dict[str,Any]]:
    ...
    iw,ih=im.size
    spec_eff=dict(spec)
    if _effective_fit(slot,spec_eff)=='cover' and spec_eff.get('focus_x') is None and spec_eff.get('focus_y') is None and bool(spec_eff.get('smart_focus',slot.get('smart_focus',slot.get('role')=='photo'))):
        fx,fy,fmeta=_smart_focus(im);spec_eff['focus_x']=fx;spec_eff['focus_y']=fy;meta.update(fmeta)
    x,y,w,h,rx,ry,rw,rh,fit=_placement(slot,iw,ih,spec_eff)
    href,_,_,meta=_encode_payload(path,im,meta,spec_eff,slot,rw,rh)
    key=str(path)+json.dumps({k:spec_eff.get(k) for k in ('fit','focus_x','focus_y','align','padding','trim','smart_focus')},sort_keys=True)
    clip_id=_safe_id(f'{template_id}-clip',slot['name'],key);img_id=_safe_id(f'{template_id}-img',slot['name'],key)
    defs=_clip(clip_id,slot,x,y,w,h);opacity=float(spec_eff.get('opacity',1.0));rotation=float(spec_eff.get('rotation',slot.get('rotation',0)) or 0);cx=float(slot['x'])+float(slot['width'])/2;cy=float(slot['y'])+float(slot['height'])/2
    body=[]
    bg=spec_eff.get('background',slot.get('background','#ffffff' if slot.get('replace_original') else None))
...
def apply_image_slots(template_id:str,source:str,config:Dict[str,Any]|None=None)->str:
    ...
    layer='<defs>'+''.join(defs)+'</defs><g id="image-slots-v16">'+''.join(body)+'</g>'
