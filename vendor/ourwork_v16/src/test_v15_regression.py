...
from ourwork_v15 import render
from reference_editable_v12 import reference_svg_path
from image_slots_v15 import list_image_slots
...
for t in REG:
 ...
 rows.append({'template_id':tid,'byte_identity':ref==out,'image_slot_count':slots['slot_count'],'auto_discovered_total':slots.get('auto_discovered_total',0)})
outdir=ROOT/'tests'/'v15';outdir.mkdir(parents=True,exist_ok=True)
...
(outdir/'v15_regression_summary.json').write_text(json.dumps(summary,indent=2),encoding='utf-8');print(summary)
