...
def _registry() -> Dict[str, Any]:
    p = ROOT / 'registry' / 'image_slots_v14.json'
...
def apply_image_slots(template_id:str,source:str,config:Dict[str,Any]|None=None)->str:
    config=config or {};images=config.get('images') or {}
    ...
    layer='<defs>'+''.join(defs)+'</defs><g id="image-slots-v14">'+''.join(body)+'</g>'
...
def image_placement_report(template_id:str,config:Dict[str,Any]|None=None)->List[Dict[str,Any]]:
    config=config or {};images=config.get('images') or {};slot_map={s['name']:s for s in list_image_slots(template_id).get('image_slots',[])};out=[]
    for name,spec in images.items():
        if name not in slot_map:continue
